...
/***/ (function(module, __webpack_exports__, __webpack_require__) {
  ...
  */
   ...
   */var Cursor=/*#__PURE__*/function(){function Cursor(options){this.options=jquery__WEBPACK_IMPORTED_MODULE_0___default.a.extend(true,{container:"body",speed:0.7,ease:"expo.out",visibleTimeout:300},options);this.body=jquery__WEBPACK_IMPORTED_MODULE_0___default()(this.options.container);this.el=jquery__WEBPACK_IMPORTED_MODULE_0___default()('<div class="cb-cursor"></div>');this.text=jquery__WEBPACK_IMPORTED_MODULE_0___default()('<div class="cb-cursor-text"></div>');this.init();}var _proto=Cursor.prototype;_proto.init=function init(){this.el.append(this.text);this.body.append(this.el);this.bind();this.move(-window.innerWidth,-window.innerHeight,0);};_proto.bind=function bind(){var _this=this;var self=this;this.body.on('mouseleave',function(){self.hide();}).on('mouseenter',function(){self.show();}).on('mousemove',function(e){_this.pos={x:_this.stick?_this.stick.x-(_this.stick.x-e.clientX)*0.15:e.clientX,y:_this.stick?_this.stick.y-(_this.stick.y-e.clientY)*0.15:e.clientY};_this.update();}).on('mousedown',function(){self.setState('-active');}).on('mouseup',function(){self.removeState('-active');}).on('mouseenter','a,input,textarea,button',function(){self.setState('-pointer');}).on('mouseleave','a,input,textarea,button',function(){self.removeState('-pointer');}).on('mouseenter','iframe',function(){self.hide();}).on('mouseleave','iframe',function(){self.show();}).on('mouseenter','[data-cursor]',function(){self.setState(this.dataset.cursor);}).on('mouseleave','[data-cursor]',function(){self.removeState(this.dataset.cursor);}).on('mouseenter','[data-cursor-text]',function(){self.setText(this.dataset.cursorText);}).on('mouseleave','[data-cursor-text]',function(){self.removeText();}).on('mouseenter','[data-cursor-stick]',function(){self.setStick(this.dataset.cursorStick);}).on('mouseleave','[data-cursor-stick]',function(){self.removeStick();});};_proto.setState=function setState(state){this.el.addClass(state);};_proto.removeState=function removeState(state){this.el.removeClass(state);};_proto.toggleState=function toggleState(state){this.el.toggleClass(state);};_proto.setText=function setText(text){this.text.html(text);this.el.addClass('-text');};_proto.removeText=function removeText(){this.el.removeClass('-text');};_proto.setStick=function setStick(el){var target=jquery__WEBPACK_IMPORTED_MODULE_0___default()(el);var bound=target.get(0).getBoundingClientRect();this.stick={y:bound.top+target.height()/2,x:bound.left+target.width()/2};this.move(this.stick.x,this.stick.y,5);};_proto.removeStick=function removeStick(){this.stick=false;};_proto.update=function update(){this.move();this.show();};_proto.move=function move(x,y,duration){gsap__WEBPACK_IMPORTED_MODULE_1__["default"].to(this.el,{x:x||this.pos.x,y:y||this.pos.y,force3D:true,overwrite:true,ease:this.options.ease,duration:this.visible?duration||this.options.speed:0});};_proto.show=function show(){var _this2=this;if(this.visible)return;clearInterval(this.visibleInt);this.el.addClass('-visible');this.visibleInt=setTimeout(function(){return _this2.visible=true;});};_proto.hide=function hide(){var _this3=this;clearInterval(this.visibleInt);this.el.removeClass('-visible');this.visibleInt=setTimeout(function(){return _this3.visible=false;},this.options.visibleTimeout);};return Cursor;}();
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('[data-magnetic]').each(function(){new magnetic__WEBPACK_IMPORTED_MODULE_2__["default"](this);});window.Magnetic=magnetic__WEBPACK_IMPORTED_MODULE_2__["default"];
